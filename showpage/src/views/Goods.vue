        ...
        <div class="data-display">
          ...
          <div v-if="!isFiltered" class="all-items-view">
            ...
            <el-table 
              ...
              <el-table-column label="操作" min-width="12%" fixed="right">
                <template #default="scope">
                  <el-button size="medium" type="warning" plain @click="handleDelete(scope.row)">出库</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <div v-else class="filtered-items-view">
            <div class="filter-header">
              <h3>查询结果</h3>
              <el-button type="primary" size="medium" @click="showAllItems">返回全部列表</el-button>
            </div>
            <el-table 
              :data="goodsList" 
              border 
  ...
  methods: {
    async fetchGoodsList() {
      try {
        ...
        console.log('Request URL:', API.GOODS.LIST)
    ...
    async submitForm() {
      try {
        ...
        if (this.dialogTitle === '新增入库') {
          await axios.post(API.GOODS.CREATE, this.goodsForm)
          this.$message.success('入库成功')
        } else {
          await axios.put(API.GOODS.UPDATE(this.goodsForm.goodsId), this.goodsForm)
    ...
    async handleDelete(row) {
      try {
        ...
        await axios.delete(API.GOODS.DELETE(row.goodsId))
    ...
    //       type: 'warning'
    //     })
    //     await axios.post(API.GOODS.OUT(row.goodsId))
    //     this.$message.success('出库成功')
    //     this.fetchGoodsList()
    //   } catch (error) {
    //     if (error !== 'cancel') {
